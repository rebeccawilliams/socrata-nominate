...
def test_parse_app_token_oneline():
    'The app token should be parsed from the minified JavaScript.'
    js = ')){$.cookies.set(\\"socrata-csrf-token\\",f)}blist.configuration.appToken=\\"U29jcmF0YS0td2VraWNrYXNz0\\";$.ajaxSetup({beforeSend:function(k){k.setRequ'
    observed = session._parse_app_token(js)
    expected = u'U29jcmF0YS0td2VraWNrYXNz0'
    n.assert_equal(observed, expected)

def test_parse_app_token_multiline():
    'The app token should be parsed from the minified JavaScript.'
    js = 'abc\n\n\ndefg\n\n)){$.cookies.set(\\"socrata-csrf-token\\",f)}blist.configuration.appToken=\\"U29jcmF0YS0td2VraWNrYXNz0\\";$.ajaxSetup({beforeSend:function(k){k.setRequ'
    observed = session._parse_app_token(js)
    expected = u'U29jcmF0YS0td2VraWNrYXNz0'
    n.assert_equal(observed, expected)
